...
public class EnemyActions : MonoBehaviour
{
    ...
    public LayerMask HitLayerMask; //the layermask for damagable objects
    ...
    [HideInInspector]
    public float zSpreadMultiplier = 3f; //multiplyer for the z distance between enemies

    [Header("Stats")]
    public RANGE range;
    public ENEMYTACTIC enemyTactic;
    public CHARACTERSTATES enemyState;
    ...
    private List<CHARACTERSTATES> NoMovementStates = new List<CHARACTERSTATES> {
        CHARACTERSTATES.DEATH,
        CHARACTERSTATES.ATTACK,
        CHARACTERSTATES.DEFEND,
        CHARACTERSTATES.GROUNDHIT,
        CHARACTERSTATES.HIT,
    ...
    private List<CHARACTERSTATES> defendableStates = new List<CHARACTERSTATES> { CHARACTERSTATES.IDLE, CHARACTERSTATES.WALK, CHARACTERSTATES.DEFEND }; //a list of states where the enemy is able to defend an incoming attack
    ...
    public void OnStart()
    {
        ...
        if (canDefendDuringAttack) defendableStates.Add(CHARACTERSTATES.ATTACK);
    ...
    {
        Debug.Log("Enemy was hit! Damage: " + d.damage);

        Debug.Log("IN HIT() Enemy state: " + enemyState);

        if (!HitableStates.Contains(enemyState))
        {
            Debug.Log("Enemy is not in a hittable state: " + enemyState);
            return;
        }
        ...
        if (HitableStates.Contains(enemyState))
        {
            ...
            if (enemyState == CHARACTERSTATES.KNOCKDOWNGROUNDED && !d.isGroundAttack) return;
            ...
            if ((enemyState == CHARACTERSTATES.KNOCKDOWNGROUNDED || enemyState == CHARACTERSTATES.GROUNDHIT) && !d.isGroundAttack)
                return;
            ...
            if (healthSystem != null)
            {
                ...
                Debug.Log("Enemy Health: " + healthSystem.currentHealth);
                if (healthSystem.currentHealth == 0)
                    return;
            }

            //ground attack
            if (enemyState == CHARACTERSTATES.KNOCKDOWNGROUNDED)
            {
                StopAllCoroutines();
                enemyState = CHARACTERSTATES.GROUNDHIT;
            ...
            else
            {
                ...
                enemyState = CHARACTERSTATES.HIT;
    ...
    void Defend()
    {
        enemyState = CHARACTERSTATES.DEFEND;
    ...
    public void CheckForHit()
    {
        ...
        Vector3 boxSize = new Vector3(lastAttack.CollSize / 1, lastAttack.CollSize / 1, hitZRange / 1);
        Collider[] hitColliders = Physics.OverlapBox(boxPosition, boxSize, Quaternion.identity, HitLayerMask);

        Debug.Log("Enemy checking for hit... HitBox Size: " + boxSize);
        ...
        while (i < hitColliders.Length)
        {
            ...
            if (damagableObject != null && damagableObject != (IDamagable<DamageObject>)this)
            {
                ...
                Debug.Log("Enemy hit: " + hitColliders[i].gameObject.name);
    ...
    {
        enemyState = CHARACTERSTATES.KNOCKDOWN;
        ...
        enemyState = CHARACTERSTATES.KNOCKDOWNGROUNDED;
        Move(Vector3.zero, 0f);
        ...
        //stand up
        enemyState = CHARACTERSTATES.STANDUP;
    ...
    public void WalkTo(float proximityRange, float movementMargin)
    {
        Vector3 dirToTarget;
        LookAtTarget(target.transform);
        enemyState = CHARACTERSTATES.WALK;
    ...
    public void Ready()
    {
        enemyState = CHARACTERSTATES.IDLE;
    ...
    public void LookAtTarget(Transform _target)
    {
        if (_target != null)
        {
            ...
            newDir = Vector3.RotateTowards(transform.forward, Vector3.right * dir, rotationSpeed * Time.deltaTime, 0.0f);
