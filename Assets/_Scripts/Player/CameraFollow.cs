...
public class CameraFollow : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
            ...
            if (targetPlayer != null) MiddlePosition = targetPlayer.transform.position;
            ...
            //Damp X
            currentX = Mathf.Lerp(currentX, MiddlePosition.x, DampX * Time.fixedDeltaTime);

            //DampY
            currentY = Mathf.Lerp(currentY, MiddlePosition.y - heightOffset, DampY * Time.fixedDeltaTime);

            //DampZ
            if (FollowZAxis)
            {
                currentZ = Mathf.Lerp(currentZ, MiddlePosition.z + distanceToTarget, DampZ * Time.fixedDeltaTime);
            }
            else
            {
