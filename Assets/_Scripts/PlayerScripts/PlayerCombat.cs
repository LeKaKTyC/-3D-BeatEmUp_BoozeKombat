...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject>
{
    ...
    private CharacterState playerState; //the state of the player
    ...
    public DamageObject lastAttack; //data from the last attack that has taken place
    ...
    public LayerMask HitLayerMask; // a list of all hittable objects
    public bool isGrounded;
    private Vector3 fixedVelocity;
    private bool updateVelocity;
    public string lastAttackInput;
    public DIRECTION lastAttackDirection;

    //a list of states when the player can attack
    private List<CHARACTERSTATES> AttackStates = new List<CHARACTERSTATES> {
        CHARACTERSTATES.IDLE,
        CHARACTERSTATES.WALK,
        CHARACTERSTATES.RUN,
        CHARACTERSTATES.JUMPING,
        CHARACTERSTATES.PUNCH,
        CHARACTERSTATES.KICK,
    ...
    private List<CHARACTERSTATES> HitableStates = new List<CHARACTERSTATES> {
        ...
        CHARACTERSTATES.KNOCKDOWNGROUNDED,
    };

    //list of states where the player can activate defence
    private List<CHARACTERSTATES> DefendStates = new List<CHARACTERSTATES> {
        CHARACTERSTATES.IDLE,
        CHARACTERSTATES.DEFEND,
        CHARACTERSTATES.WALK,
        CHARACTERSTATES.RUN,
    ...
    void Start()
    {
        ...
        {
            HitableStates.Add(CHARACTERSTATES.JUMPING);
            HitableStates.Add(CHARACTERSTATES.JUMPKICK);
        }
    }
    ...
    private void OnInputEvent(string action, BUTTONSTATE buttonState)
    {
        if (AttackStates.Contains(playerState.currentState) && !isDead)
        {
            ...
            //running punch
            if (action == "Punch" && buttonState == BUTTONSTATE.PRESS && playerState.currentState == CHARACTERSTATES.RUN && isGrounded)
            {
                animator.SetAnimatorBool("Run", false);
                if (RunningPunch.animTrigger.Length > 0) doAttack(RunningPunch, CHARACTERSTATES.ATTACK, "Punch");
                return;
            }

            //running kick
            if (action == "Kick" && buttonState == BUTTONSTATE.PRESS && playerState.currentState == CHARACTERSTATES.RUN && isGrounded)
            {
                animator.SetAnimatorBool("Run", false);
                if (RunningKick.animTrigger.Length > 0) doAttack(RunningKick, CHARACTERSTATES.ATTACK, "Kick");
                return;
            }
            ...
            //ground punch
            if (action == "Punch" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState != CHARACTERSTATES.PUNCH && NearbyEnemyDown()) && isGrounded)
            {
                if (GroundPunchData.animTrigger.Length > 0) doAttack(GroundPunchData, CHARACTERSTATES.GROUNDPUNCH, "Punch");
                return;
            }

            //ground kick
            if (action == "Kick" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState != CHARACTERSTATES.KICK && NearbyEnemyDown()) && isGrounded)
            {
                if (GroundKickData.animTrigger.Length > 0) doAttack(GroundKickData, CHARACTERSTATES.GROUNDKICK, "Kick");
                return;
            }
            ...
            //default punch
            if (action == "Punch" && buttonState == BUTTONSTATE.PRESS && playerState.currentState != CHARACTERSTATES.PUNCH && playerState.currentState != CHARACTERSTATES.KICK && isGrounded)
            {
                ...
                if (PunchCombo[attackNum] != null && PunchCombo[attackNum].animTrigger.Length > 0) doAttack(PunchCombo[attackNum], CHARACTERSTATES.PUNCH, "Punch");
                return;
            }

            //advance the punch combo if "punch" was pressed during a punch attack
            if (action == "Punch" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState == CHARACTERSTATES.PUNCH) && !continuePunchCombo && isGrounded)
            {
            ...
            //default kick
            if (action == "Kick" && buttonState == BUTTONSTATE.PRESS && playerState.currentState != CHARACTERSTATES.KICK && playerState.currentState != CHARACTERSTATES.PUNCH && isGrounded)
            {
            ...
            if (action == "Kick" && buttonState == BUTTONSTATE.PRESS && (playerState.currentState == CHARACTERSTATES.KICK) && !continueKickCombo && isGrounded)
            {
    ...
    private void doAttack(DamageObject damageObject, CHARACTERSTATES state, string inputAction)
    {
        ...
        if (state == CHARACTERSTATES.JUMPKICK) return;
    ...
    {
        playerState.SetState(CHARACTERSTATES.USEWEAPON);
        TurnToDir(currentDirection);
    ...
    IEnumerator JumpKickInProgress()
    {
        ...
        //check for hit
        while (playerState.currentState == CHARACTERSTATES.JUMPKICK)
        {
    ...
    private void Defend(bool defend)
    {
        ...
        if (defend)
        {
            TurnToDir(currentDirection);
            SetVelocity(Vector3.zero);
            playerState.SetState(CHARACTERSTATES.DEFEND);
            animator.SetAnimatorBool("Run", false); //disable running

        }
        else
        {
            if (playerState.currentState == CHARACTERSTATES.DEFEND) playerState.SetState(CHARACTERSTATES.IDLE);
        }
    }
    ...
    public void CheckForHit()
    {
        ...
        Debug.Log("Checking for hit... HitBox Size: " + boxSize);
    ...
    public void Hit(DamageObject d)
    {
        ...
        {
            Debug.Log("Player is not in a hittable state: " + playerState.currentState);
            return;
        }
        ...
        if (HitableStates.Contains(playerState.currentState))
        {
            ...
            if (playerState.currentState == CHARACTERSTATES.DEFEND && !d.DefenceOverride && (isFacingTarget(d.inflictor) || blockAttacksFromBehind))
            {
                Defend(d);
                return;
            }
            ...
            {
                healthSystem.TakeDamage(d.damage);
            ...
            //check for knockdown
            if ((hitKnockDownCount >= knockdownHitCount || !IsGrounded() || d.knockDown) && playerState.currentState != CHARACTERSTATES.KNOCKDOWN)
            {
                hitKnockDownCount = 0;
    ...
    public void interactWithItem()
    {
        if (itemInRange != null)
        {
            ...
            playerState.SetState(CHARACTERSTATES.PICKUPITEM);
            SetVelocity(Vector3.zero);
            Invoke("Ready", .3f);
    ...
    {
        playerState.SetState(CHARACTERSTATES.KNOCKDOWN);
        ...
        playerState.currentState = CHARACTERSTATES.STANDUP;

        yield return new WaitForSeconds(KnockdownStandUpTime);
        playerState.currentState = CHARACTERSTATES.IDLE;
    }
    ...
    bool NearbyEnemyDown()
    {
        ...
        if (closestEnemy != null)
        {
            ...
            if (AI != null && AI.enemyState == CHARACTERSTATES.KNOCKDOWNGROUNDED)
            {
                return true;
            }
        }
    ...
    public void Ready()
    {
        ...
        if (continueKickCombo)
        {
            ...
            if (KickCombo[attackNum] != null && KickCombo[attackNum].animTrigger.Length > 0) doAttack(KickCombo[attackNum], CHARACTERSTATES.KICK, "Kick");
            return;
        }
    ...
    {
        transform.rotation = Quaternion.LookRotation(Vector3.right * (int)dir);
    }
